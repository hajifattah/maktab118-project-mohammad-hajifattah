...
export const ShoppingCardItem: React.FC<IShopping> = ({
  ...
  return (
    <>
      {" "}
      <div className="relative group-hover:blur-sm w-full sm:w-[18%] sm:max-w-[13rem] aspect-square">
        <Image
          alt="image"
      ...
      <div className="grid sm:hidden w-full px-1">
        ...
        <div className="flex justify-between items-center gap-x-3 w-full mt-1">
          <p className="font font-semibold w-1/4 flex-auto ">{price} تومان</p>
          <p className="font-semibold w-1/4 flex-auto">{total} تومان</p>
      ...
      {/* after sm screen */}
      <Link href={`/products/${id}`} className="w-[18%] hidden sm:block">
        <h2 className="text-sm font-semibold hidden max-w-[14rem] truncate sm:block ">
          {title}
        </h2>
      </Link>
      <p className="font font-semibold w-[18%] hidden sm:block">
        {price} تومان
      </p>
      <div className="w-[25%] max-w-[7.15rem] lg:max-w-[8.4rem] lg:ml-20 hidden sm:block">
        <ChangeQuantity changeqty={changeQty} quantity={qty} />
      </div>
      <p className="font font-semibold w-[18%] hidden sm:block">
        {total} تومان
